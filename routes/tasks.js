...
const { validateCreateTask, validateUpdateTask } = require('../validators/tasks');
 ...
 */
router.post('/', middlewareAuthentication, validateCreateTask, async (req, res) => {
 ...
 */
router.patch('/:taskId', middlewareAuthentication, validateUpdateTask, async (req, res) => {
	if (validationResultCheck(req, res)) {
		return;
	}
    ...
    await task.update({ title, concluded });
